const { Tipo_material, Familia_material } = require('../models');
...
module.exports = {
    ...
    createOrUpdate: async (req, res) => {
        try {
            ...
            if (codigo_tipo_material == 0 || codigo_tipo_material === undefined) {
                // Create new record
                const newRecord = await Tipo_material.create({
                ...
                const response = await Tipo_material.findByPk(newRecord.codigo_tipo_material, {
                    include: [
                ...
                const record = await Tipo_material.findByPk(codigo_tipo_material, {
                    include: [
                        { model: Familia_material, as: 'familia_material' }
                    ]
                });
                ...
                if (nombre_tipo_material !== undefined) record.nombre_tipo_material = nombre_tipo_material;
                ...
                const updatedRecord = await Tipo_material.findByPk(codigo_tipo_material, {
                    include: [
                        { model: Familia_material, as: 'familia_material' }
                    ]
                });
    ...
    getAll: async (req, res) => {
        try {
            ...
            const { count, rows } = await Tipo_material.findAndCountAll({
    ...
    getById: async (req, res) => {
        try {
            ...
            const record = await Tipo_material.findByPk(id, {
                include: [
                    { model: Familia_material, as: 'familia_material' }
                ]
            });
